...
/**
 * 此类提供下载笔趣阁（https://www.xbiquge6.com/）的小说
 */
public class XBiQuGe implements BookDownloader {
    // 书的链接
     ...
     */
    public XBiQuGe(String url, Saver saver){
        this.url = url;
        this.saver = saver;
    }
    ...
    private static void getBookPages() {
        try {
            ...
            for (Element ele : a) {
                // 将章节标题及链接保存
                titles.add(ele.text());
                urls.add(R.url.新笔趣阁 + ele.attr("href"));
//                 System.out.printf("Find -> %s and url -> %s\n", ele.text(), Urls.XBiQuGe + ele.attr("href"));
                // Thread.sleep(1);
            }
        } catch (IOException e) {
            e.printStackTrace();
